...
__global__ void compute_output_gradients_kernel(float *grad_output, float *output, int *labels, int batch_size) {
    ...
    if (b < batch_size) {
        for (int i = 0; i < OUTPUT_SIZE; ++i) {
            grad_output[b * OUTPUT_SIZE + i] = output[b * OUTPUT_SIZE + i]; //copy softmax output
        }
        grad_output[b * OUTPUT_SIZE + labels[b]] -= 1.0f; // one hot encoding for the correct label
        
        // Divide by batch_size
        for (int i = 0; i < OUTPUT_SIZE; ++i) {
            grad_output[b * OUTPUT_SIZE + i] /= batch_size; // normalize by batch size as loss is per batch here
        }
    }
}
...
void forward_timed(NeuralNetwork *nn, float *input, float *hidden, float *output, int batch_size, TimingStats *stats) {
    ...
    dim3 block_size(32, 32); // block size 

...
#Loading binary data
X_train = np.fromfile('data/X_train.bin', dtype=np.float32).reshape(60000,28*28)
y_train = np.fromfile('data/y_train.bin', dtype=np.int32)
X_test = np.fromfile('data/X_test.bin',dtype=np.float32).reshape(10000,28*28)
y_test = np.fromfile('data/y_test.bin', dtype=np.int32)

# Applying MNIST normalization
mean, std = 0.1307, 0.3081
X_train_np = (X_train - mean) / std
X_test_np = (X_test - mean) / std

# Convert to Pytorch tensors and move to GPU. on my mac it's mps
train_data = torch.from_numpy(X_train_np[:TRAIN_SIZE]).reshape(-1,1,28,28).to(device=device)
train_labels = torch.from_numpy(y_train[:TRAIN_SIZE]).long().to(device=device)
test_data = torch.from_numpy(X_test_np).reshape(-1,1,28,28).to(device=device)
test_labels = torch.from_numpy(y_test).long().to(device=device)
...
model = MLP(28*28, 1024,10).to(device=device)
...
def evaluate(model, test_data, test_labels):
    ...
    with torch.no_grad():
        for i in range(len(test_data) // batch_size): #test data per batch
